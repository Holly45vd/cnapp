...
(function(Probability$1) {
	Probability$1[Probability$1["Unknown"] = 1e-13] = "Unknown";
	Probability$1[Probability$1["Rule"] = 1e-12] = "Rule";
	Probability$1[Probability$1["DICT"] = 2e-8] = "DICT";
	Probability$1[Probability$1["Surname"] = 1] = "Surname";
	Probability$1[Probability$1["Custom"] = 1] = "Custom";
...
var FastDictFactory = class {
	...
	set(word, pinyin$1) {
		if (word.length > 1) this.StringDICT.set(word, pinyin$1);
		else {
			const code = word.charCodeAt(0);
			this.NumberDICT[code] = pinyin$1;
		}
	}
	clear() {
		this.NumberDICT = [];
...
function processToneSandhi(cur, pre, next) {
	...
	if (next && !toneSandhiIgnoreSuffix[cur].includes(next)) {
		...
		if (nextPinyin !== next) {
			...
			for (let pinyin$1 in pinyinMap) if (pinyinMap[pinyin$1].indexOf(Number(nextTone)) !== -1) return pinyin$1;
		}
	}
}
...
(function(TokenizationAlgorithm$1) {
	TokenizationAlgorithm$1[TokenizationAlgorithm$1["ReverseMaxMatch"] = 1] = "ReverseMaxMatch";
	TokenizationAlgorithm$1[TokenizationAlgorithm$1["MaxProbability"] = 2] = "MaxProbability";
	TokenizationAlgorithm$1[TokenizationAlgorithm$1["MinTokenization"] = 3] = "MinTokenization";
	...
	match(text, surname) {
		...
		for (let i = 0; i < zhChars.length; i++) {
			...
			if (cur === null) cur = this.root;
				...
				while (failNode !== null) {
					const pattern$1 = failNode.patterns.find((item) => {
						if (surname === "off") return item.priority !== Priority.Surname;
						else if (surname === "head") return item.length - 1 - i === 0;
						else return true;
					});
					if (pattern$1) result.push(Object.assign(Object.assign({}, pattern$1), { index: i - pattern$1.length + 1 }));
...
function addCustomConfigToDict(config, dict, handleType$1) {
	for (let word in config) {
		const pinyins = config[word];
		splitString(word).forEach((char, index) => {
			const pinyin$1 = pinyins.split(" ")[index] || "";
			if (handleType$1 === "replace" || handleType$1 === "add" && !dict.get(char) && !DICT1.get(char)) dict.set(char, pinyin$1);
			else {
				dict.set(char, dict.get(char) || DICT1.get(char));
				if (!dict.get(char).split(" ").includes(pinyin$1)) dict.set(char, `${dict.get(char)} ${pinyin$1}`.trim());
			}
		});
	}
}
...
var getSingleWordPinyin = (char) => {
	const pinyin$1 = DICT1.get(char);
	return pinyin$1 ? pinyin$1.split(" ")[0] : char;
};
var getPinyin = (word, list, surname, segmentit) => {
	...
	for (let i = 0; i < zhChars.length;) {
		const match$1 = matches[matchIndex];
		if (match$1 && i === match$1.index) {
			if (match$1.length === 1 && match$1.priority <= Priority.Normal) {
				const char = zhChars[i];
				let pinyin$1 = "";
				pinyin$1 = processSepecialPinyin(char, zhChars[i - 1], zhChars[i + 1]);
				list[i] = {
					origin: char,
					result: pinyin$1,
					isZh: pinyin$1 !== char,
					originPinyin: pinyin$1
				};
				i++;
				matchIndex++;
				continue;
			}
			const pinyins = match$1.pinyin.split(" ");
			let pinyinIndex = 0;
			for (let j = 0; j < match$1.length; j++) {
				const zhChars$1 = splitString(match$1.zh);
				list[i + j] = {
					origin: zhChars$1[j],
...
var getPinyinWithoutTone = (pinyin$1) => {
	return pinyin$1.replace(/(ā|á|ǎ|à)/g, "a").replace(/(ō|ó|ǒ|ò)/g, "o").replace(/(ē|é|ě|è)/g, "e").replace(/(ī|í|ǐ|ì)/g, "i").replace(/(ū|ú|ǔ|ù)/g, "u").replace(/(ǖ|ǘ|ǚ|ǜ)/g, "ü").replace(/(n̄|ń|ň|ǹ)/g, "n").replace(/(m̄|ḿ|m̌|m̀)/g, "m").replace(/(ê̄|ế|ê̌|ề)/g, "ê");
};
var getAllPinyin = (char, surname = "off") => {
	const customMultpileDict = getCustomMultpileDict();
	let pinyin$1 = DICT1.get(char) ? DICT1.get(char).split(" ") : [];
	if (customMultpileDict.get(char)) pinyin$1 = customMultpileDict.get(char).split(" ");
	else if (surname !== "off") {
		const surnamePinyin = Surnames[char];
		if (surnamePinyin) pinyin$1 = [surnamePinyin].concat(pinyin$1.filter((py) => py !== surnamePinyin));
	}
	return pinyin$1;
};
var getMultiplePinyin = (word, surname = "off") => {
	let pinyin$1 = getAllPinyin(word, surname);
	if (pinyin$1.length > 0) return pinyin$1.map((value) => ({
		origin: word,
		result: value,
		isZh: true,
		originPinyin: value
...
var getInitialAndFinal = (pinyin$1, initialPattern) => {
	const pinyin_arr = pinyin$1.split(" ");
	const initial_arr = [];
	const final_arr = [];
...
var getFinalParts = (pinyin$1) => {
	const { final } = getInitialAndFinal(pinyin$1);
	let head = "", body = "", tail = "";
...
var getNumOfTone = (pinyin$1) => {
	...
	const tone_num_arr = [];
	pinyin$1.split(" ").forEach((_pinyin) => {
...
var getPinyinWithNum = (pinyin$1, originPinyin) => {
	const pinyin_arr = getPinyinWithoutTone(pinyin$1).split(" ");
...
var getFirstLetter = (pinyin$1, isZh) => {
	const first_letter_arr = [];
	pinyin$1.split(" ").forEach((pinyin$2) => {
		first_letter_arr.push(isZh ? pinyin$2[0] : pinyin$2);
	});
...
var middlewareType = (list, options, word) => {
	...
	if (options.type === "all") return list.map((item) => {
		const pinyin$1 = item.isZh ? item.result : "";
		const { initial, final } = getInitialAndFinal(pinyin$1, options.initialPattern);
		const { head, body, tail } = getFinalParts(pinyin$1);
		let polyphonic$1 = [];
		if (pinyin$1 !== "") polyphonic$1 = [pinyin$1].concat(getAllPinyin(item.origin, options.surname).filter((item$1) => item$1 !== pinyin$1));
		return {
			origin: item.origin,
			pinyin: pinyin$1,
			initial,
...
function pinyin(word, options) {
	...
	let { list } = getPinyin(word, Array(stringLength(word)), options.surname, options.segmentit);
...
function addDict(dict, options) {
	...
	for (let word in dict) {
		...
		const pinyin$1 = Array.isArray(value) ? value[0] : value;
		const wordLength = stringLength(word);
		if (wordLength === 1) addToOriginDict(dictName, word, pinyin$1, dict1Handle);
...
function addToOriginDict(dict, char, pinyin$1, handle = "add") {
	...
	if (handle === "add") {
		...
		if (existedPinyin && !existedPinyin.split(" ").includes(pinyin$1)) DICT1.set(char, `${existedPinyin} ${pinyin$1}`);
		else if (!DICT1.get(char)) DICT1.set(char, pinyin$1);
	} else if (handle === "replace") DICT1.set(char, pinyin$1);
}
...
*/
var match = (text, pinyin$1, options) => {
	if ((options === null || options === void 0 ? void 0 : options.precision) === "any") options.lastPrecision = "any";
	if (options === null || options === void 0 ? void 0 : options.v) pinyin$1 = pinyin$1.replace(/ü/g, "v");
	const completeOptions = Object.assign(Object.assign({}, DefaultMatchOptions), options || {});
	if (completeOptions.insensitive !== false) {
		text = text.toLowerCase();
		pinyin$1 = pinyin$1.toLowerCase();
	}
	if (completeOptions.space === "ignore") pinyin$1 = pinyin$1.replace(/\s/g, "");
	const result = (options === null || options === void 0 ? void 0 : options.precision) === "any" ? matchAny(text, pinyin$1, completeOptions) : matchAboveStart(text, pinyin$1, completeOptions);
...
function polyphonic(text, options = DEFAULT_OPTIONS$1) {
	...
	doubleList.forEach((list$1) => {
		middlewarePattern(list$1, options);
	});
	doubleList.forEach((list$1) => {
		middlewareToneType(list$1, options);
	});
	doubleList.forEach((list$1) => {
		middlewareV(list$1, options);
	});
	return doubleList.map((list$1) => handleType(list$1, options));
}
var getPolyphonicList = (text) => {
	return splitString(text).map((char) => {
		const pinyin$1 = getCustomPolyphonicDict().get(char) || DICT1.get(char) || char;
		return {
			origin: char,
			result: pinyin$1,
			isZh: pinyin$1 !== char,
...
var getSplittedPolyphonicList = (list) => {
	return list.map((item) => {
		return item.isZh ? item.result.split(" ").map((pinyin$1) => ({
			origin: item.origin,
			result: pinyin$1,
			isZh: true,
			originPinyin: pinyin$1
...
*/
function convert(pinyin$1, options) {
	options = Object.assign(Object.assign({}, DefaultConvertOptions), options || {});
	const originType = typeof pinyin$1;
	if (typeof pinyin$1 === "string") pinyin$1 = pinyin$1.split(options.separator);
	pinyin$1 = pinyin$1.map((item) => {
	...
	if (originType === "string") return pinyin$1.join(options.separator);
	else return pinyin$1;
}
function formatNumToSymbol(pinyin$1) {
	const lastChar = Number(pinyin$1[pinyin$1.length - 1]);
	if (lastChar >= 0 && lastChar <= 4) {
		for (let key in toneMap) if (pinyin$1.includes(key)) return pinyin$1.slice(0, pinyin$1.length - 1).replace(key, toneMap[key][lastChar]);
		return pinyin$1;
	} else return pinyin$1;
}
function formatSymbolToNum(pinyin$1) {
	return `${getPinyinWithoutTone(pinyin$1)}${getNumOfTone(pinyin$1)}`;
}
function formatToneNone(pinyin$1) {
	return getPinyinWithoutTone(pinyin$1);
}
var OutputFormat;
(function(OutputFormat$1) {
	OutputFormat$1[OutputFormat$1["AllSegment"] = 1] = "AllSegment";
	OutputFormat$1[OutputFormat$1["AllArray"] = 2] = "AllArray";
	OutputFormat$1[OutputFormat$1["AllString"] = 3] = "AllString";
	OutputFormat$1[OutputFormat$1["PinyinSegment"] = 4] = "PinyinSegment";
	OutputFormat$1[OutputFormat$1["PinyinArray"] = 5] = "PinyinArray";
	OutputFormat$1[OutputFormat$1["PinyinString"] = 6] = "PinyinString";
	OutputFormat$1[OutputFormat$1["ZhSegment"] = 7] = "ZhSegment";
	OutputFormat$1[OutputFormat$1["ZhArray"] = 8] = "ZhArray";
	OutputFormat$1[OutputFormat$1["ZhString"] = 9] = "ZhString";
...
function middlewareSegment(list, matches) {
	...
	while (i < list.length && j < matches.length) {
		const match$1 = matches[j];
		const item = list[i];
		if (match$1.zh.startsWith(item.origin)) {
			const start = i;
			const chars = splitString(match$1.zh);
			let cur = start + 1;
			while (cur < list.length && list[cur].origin === chars[cur - start]) cur++;
			const _segment = list.slice(start, cur);
			segments.push({
				segment: _segment.map((item$1) => ({
...
function middlewareOutputFormat(segments, options) {
	...
	if (format === OutputFormat.AllSegment) return segments.map((item) => {
		return {
			origin: item.segment.map((item$1) => item$1.origin).join(""),
			result: item.segment.map((item$1) => item$1.result).join("")
		};
	});
	...
	} else if (format === OutputFormat.PinyinSegment) return segments.map((item) => item.segment.map((item$1) => item$1.result).join(""));
	else if (format === OutputFormat.PinyinArray) return segments.map((item) => item.segment.map((item$1) => item$1.result));
	else if (format === OutputFormat.PinyinString) return segments.map((item) => item.segment.map((item$1) => item$1.result).join("")).join(separator);
	else if (format === OutputFormat.ZhSegment) return segments.map((item) => item.segment.map((item$1) => item$1.origin).join(""));
	else if (format === OutputFormat.ZhArray) return segments.map((item) => item.segment.map((item$1) => item$1.origin));
	else if (format === OutputFormat.ZhString) return segments.map((item) => item.segment.map((item$1) => item$1.origin).join("")).join(separator);
}
var DEFAULT_OPTIONS = {
	toneType: "symbol",
	mode: "normal",
...
function segment(word, options) {
	...
	let { list, matches } = getPinyin(word, Array(stringLength(word)), options.surname, options.segmentit);
	list = middlewareToneSandhi(list, options.toneSandhi);
	list = middleWareNonZh(list, options);
	middlewareToneType(list, options);
	middlewareV(list, options);
